...
def make_vhdl(instr_dict):
    names_str = ''
    for i in instr_dict:
        names_str += f"  constant opcode_{i.replace('.','_'):<18s} : std_ulogic_vector(31 downto 0) := \"{instr_dict[i]['encoding']}\";\n"
    names_str += '  -- Instruction mask and match (use opcode_xxx and case? if you use std>=08)\n'
    for i in instr_dict:
        names_str += f'  constant match_{i.upper().replace(".","_"):<18s}: std_ulogic_vector(31 downto 0) := \"{instr_dict[i]['encoding'].replace('-', '0')}\";\n'
        names_str += f'  constant mask_{i.upper().replace(".","_"):<19s}: std_ulogic_vector(31 downto 0) := \"{instr_dict[i]['encoding'].replace("0", '1').replace('-', '0')}\";\n'
    names_str += '  -- CSR Addresses\n'
    for num, name in csrs+csrs32:
        names_str += f"  constant csr_{name:<18s} : std_ulogic_vector(11 downto 0) := x\"{("{:03x}".format(num))}\";\n"
    
    names_str += '  -- Causes\n'
    for num, name in causes:
        names_str += f'  constant cause_{name.replace(" ","_"):<26s}: natural := {num};\n'

    vhdl_file = open('inst.vhd','w')
    vhdl_file.write(f'''
-- Automatically generated by parse_opcodes
library ieee;
use ieee.std_logic_1164.all;

package riscv_instr is
{names_str}
end riscv_instr;
''')
    vhdl_file.close()
...
if __name__ == "__main__":
    ...
    for i in ['-c','-latex','-chisel','-sverilog','-rust', '-go', '-spinalhdl', '-vhdl']:
        if i in extensions:
            extensions.remove(i)
    ...
    if '-vhdl' in sys.argv[1:]:
        make_vhdl(instr_dict)
        logging.info('inst.vhd generated successfully')
